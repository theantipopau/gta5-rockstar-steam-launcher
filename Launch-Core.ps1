<#
    Rockstar Games Launcher -> Steam auto-launch engine
    github.com/theantipopau/gta5-rockstar-steam-launcher
    Created by Matt Hurley - matthurley.dev

    This file is NOT meant to be pointed at directly from Steam. It's shared logic used
    by the small per-game wrapper scripts (LaunchGTAV.ps1, LaunchRDR2.ps1, etc.), which
    set $GameProcessName and $TemplateImageName before dot-sourcing this file. To add
    support for another Rockstar title, copy one of the wrapper scripts and change those
    two values -- see the README.
#>
...
Log "=== Script started (gta5-rockstar-steam-launcher / Launch-Core.ps1) ==="

if ([string]::IsNullOrWhiteSpace($GameProcessName)) {
    Log "FATAL: `$GameProcessName was not set by the wrapper script that dot-sourced this file. Nothing to do."
    exit 1
}
if ([string]::IsNullOrWhiteSpace($TemplateImageName)) {
    Log "FATAL: `$TemplateImageName was not set by the wrapper script that dot-sourced this file. Nothing to do."
    exit 1
}

Log "GameProcessName = $GameProcessName"
Log "TemplateImageName = $TemplateImageName"

$launcherExe = "C:\Program Files\Rockstar Games\Launcher\Launcher.exe"
$templatePath = Join-Path $scriptDir $TemplateImageName

# Everything from here on is wrapped so that ANY unexpected failure gets logged with full
# detail instead of the window just flashing closed with no trace of what happened.
try {

if (-not (Test-Path $launcherExe)) {
    Log "FATAL: Rockstar Games Launcher not found at '$launcherExe'. If it's installed somewhere"
    Log "else, edit `$launcherExe near the top of Launch-Core.ps1."
    exit 1
}
...
Log "Waiting for $GameProcessName to appear..."
$deadline = (Get-Date).AddSeconds(180)
$gameProc = $null
while ((Get-Date) -lt $deadline) {
    $gameProc = Get-Process -Name $GameProcessName -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($gameProc) { break }
    Start-Sleep -Seconds 1
}
if ($gameProc) {
    Log "$GameProcessName detected (PID $($gameProc.Id))"
} else {
    Log "$GameProcessName never appeared within 180s -- check that `$GameProcessName in the wrapper"
    Log "script matches the ACTUAL running process name (Task Manager > Details tab, while the"
    Log "game is running, without the .exe extension)."
}
...
Log "=== Script finished normally ==="
    ...
    Log "=== UNHANDLED ERROR: $($_.Exception.GetType().FullName): $($_.Exception.Message) ==="
    Log "At: $($_.InvocationInfo.PositionMessage)"
    Log "Stack trace:`n$($_.ScriptStackTrace)"
    exit 1
}
